defmodule Fluffy.Conformance.NetworkEventTest do
  ...
  test "captures a predicate-matched browser request" do
    ...
    session =
      ...
      |> expect(Fluffy.Expect.request_to_have_method(:save_request, "POST"))
      |> expect(Fluffy.Expect.request_to_have_url(:save_request, TestHTTPFixtures.url(fixture, "/api?source=button")))
      |> expect(Fluffy.Expect.request_to_have_headers(:save_request, %{"x-fluffy" => "network-test"}))
      |> expect(Fluffy.Expect.request_to_have_resource_type(:save_request, "fetch"))
      |> expect(Fluffy.Expect.request_to_have_post_data(:save_request, "payload"))
      |> expect(Fluffy.Expect.request_to_have_page(:save_request, :main))
  ...
  test "captures a regex-matched browser response" do
    ...
    session =
      ...
      |> expect(Fluffy.Expect.response_to_have_request_method(:save_response, "POST"))
      |> expect(Fluffy.Expect.response_to_have_url(:save_response, TestHTTPFixtures.url(fixture, "/api?source=button")))
      |> expect(Fluffy.Expect.response_to_have_headers(:save_response, %{"content-type" => "application/json"}))
      |> expect(Fluffy.Expect.response_to_have_resource_type(:save_response, "fetch"))
      |> expect(Fluffy.Expect.response_to_have_status(:save_response, 207))
      |> expect(Fluffy.Expect.response_to_have_status_text(:save_response, "Multi-Status"))
      |> expect(Fluffy.Expect.response_to_have_page(:save_response, :main))
